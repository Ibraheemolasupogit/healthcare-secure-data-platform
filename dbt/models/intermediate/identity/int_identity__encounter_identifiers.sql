...
with synthetic_encounters as (

    select
        'MILESTONE_2_SYNTHETIC' as source_namespace,
        encounter_id as source_encounter_identifier,
        encounter_id as canonical_encounter_id,
        source_record_id,
        source_relation,
        source_system,
        source_updated_at,
    ...
    from {{ ref('stg_clinical__encounters') }}

), interoperability_crosswalks as (

    select
        concat(source_format, ':', source_system) as source_namespace,
        source_identifier as source_encounter_identifier,
        canonical_identifier as canonical_encounter_id,
        source_record_id,
        source_relation,
        source_system,
    ...
    where identifier_type = 'ENCOUNTER'

), ingestion_envelope_identifiers as (

    select
        concat(source_format, ':', source_system) as source_namespace,
        source_encounter_id as source_encounter_identifier,
        coalesce(canonical_encounter_id, source_encounter_id) as canonical_encounter_id,
        source_record_id,
        source_relation,
        source_system,
        ...
        iff(canonical_encounter_id is null and source_encounter_id is not null, true, false) as is_unmatched,
        false as is_conflicting
    ...
    select * from synthetic_encounters
    union all
    ...
    select
        source_namespace,
        source_encounter_identifier,
        count(distinct canonical_encounter_id) as canonical_encounter_count
    from unioned
    group by source_namespace, source_encounter_identifier

)

select
    unioned.source_namespace,
    unioned.source_encounter_identifier,
    unioned.canonical_encounter_id,
    {{ generate_healthcare_surrogate_key(['unioned.canonical_encounter_id']) }} as encounter_key,
    ...
    iff(duplicate_conflicts.canonical_encounter_count > 1, true, unioned.is_conflicting) as is_conflicting
from unioned
left join duplicate_conflicts
    on unioned.source_namespace = duplicate_conflicts.source_namespace
    and unioned.source_encounter_identifier = duplicate_conflicts.source_encounter_identifier
