...
with source_rows as (
    ...
    select *
    from {{ source('raw_operational', 'providers') }}
    ...
    {{ normalise_empty_string('provider_label') }} as provider_label,
    {{ normalise_code('provider_role') }} as provider_role,
    ...
    coalesce(to_varchar(provider_id), '') as source_record_id,
    '{{ source('raw_operational', 'providers') }}' as source_relation,
    ...
    select *
    from typed
    {{ deduplicate_source("provider_id", "valid_from desc nulls last, provider_id asc") }}
