...
| **4 — FHIR and HL7 ingestion (COMPLETE LOCALLY)** | Synthetic messages, simplified validation, identifier/terminology mapping, immutable RAW contracts and rejected-message quarantine | 24 accepted positive payloads; eight expected failures quarantined; deterministic checksums | Live clinical interfaces and formal conformance |
| **5 — dbt staging layer (COMPLETE LOCALLY)** | Sources/freshness and source-aligned staging for batch/FHIR/HL7/audit/quarantine/governance inputs | 23 sources, 23 staging views, dbt parse, static guardrails and evidence; live Snowflake build pending credentials | Conformed core and marts |
| **6 — Healthcare core model (COMPLETE LOCALLY)** | Conformed patient identity, patient, organisation, provider, location, encounter, admission, appointment, pathway, clinical event, pathology, medication, consent and research eligibility foundation | dbt parse, static lineage/boundary tests, contracts and evidence; live Snowflake build pending credentials | Billing/finance logic |
| **7 — Billing and finance synthetic extension (COMPLETE LOCALLY)** | Versioned services, products, tariffs, contracts, claims, invoices/lines, payment lifecycle, exceptions and control totals in the existing generator | Determinism, keys, lifecycle/amount rules, checksums and bounded committed samples | dbt billing models or real finance data |
| **8 — Billing and finance dbt domain (COMPLETE LOCALLY)** | Billable activity, effective tariffs, invoice totals, payment allocation, refunds/adjustments, outstanding balance and revenue models | Known-case unit tests, source/control reconciliation, contracts and lineage | Revenue-assurance workflow orchestration |
| **9 — Reconciliation and revenue assurance (COMPLETE LOCALLY)** | Control results, exception classifications, owner/priority/remediation state, revenue-at-risk summaries and deterministic evidence metadata over M8 outputs | dbt parse, static guardrails, local evidence pack and declared contracts; live Snowflake build pending credentials | BI dashboards, external workflows, formal finance close and ML |
| **10 — Airflow orchestration (COMPLETE LOCALLY)** | Cross-platform DAGs, sensors, retries, backfills, failure callbacks, command wrappers and local evidence metadata | Static DAG structure, dependency, configuration, idempotency, callback and scope guardrails; optional scheduler execution deferred | Reimplementing dbt DAGs, Snowflake Tasks or later-platform workflows |
...
| **15 — Protected CI/CD and deployment controls (COMPLETE LOCALLY)** | GitHub Actions guardrails, environment promotion model, Terraform validation/plan contracts, release/deployment manifests, policy gates, drift simulation, rollback design and evidence | Workflow-permission tests, registry validation, promotion/order checks, checksum evidence and local drift simulation | Live apply, production deployment or automatic rollback |
| **16 — Multi-region architecture and recovery (COMPLETE LOCALLY)** | Symbolic regional design, residency constraints, recovery tiers, RTO/RPO targets, dependency mapping, failover/failback controls, local simulation and evidence | Registry validation, dependency graph checks, scenario tests, recovery manifest and checksum evidence | Live failover, live replication or production resilience claims |
| **17 — Operational observability and incident readiness (COMPLETE LOCALLY)** | Service catalogue, health model, health checks, SLIs, synthetic SLOs, error budgets, incident taxonomy, routing metadata, runbooks, recovery-drill catalogue, local simulations and evidence | Registry validation, deterministic health/incident/drill simulations, checksum evidence, CLI and CI guardrails | Live monitoring, live alerts, ticketing integrations, automatic remediation, production SLO claims or live failover |
| **18 — Enterprise integration and portfolio release-readiness (COMPLETE LOCALLY)** | Golden-path demo, capability matrix, technology matrix, ownership matrix, evidence index, claim validation, local v1.0 release manifest and reviewer documentation | `healthcare-platform demo run-golden-path`, release-readiness validation, checksum evidence and Milestone 18 tests | Live deployment, fabricated screenshots, GitHub release creation, production/compliance/clinical claims or new technology domains |
